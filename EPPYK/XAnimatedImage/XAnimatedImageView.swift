...
class XAnimatedImageView: UIImageView {
    ...
    weak var delegate: XAnimatedImageViewDelegate?
    ...
    override func startAnimating() {
        ...
        if let delegate = self.delegate {
            delegate.xAnimationDidStart(self.animatedImage)
        }
    }
            ...
            while self.accumulator >= NSTimeInterval(delayTime) {
                ...
                if self.currentFrameIndex >= self.animatedImage.frameCount {
                    ...
                    if self.loopCountdown == 0 {
                        self.stopAnimating()
                        if let delegate = self.delegate {
                            delegate.xAnimationDidStop(self.animatedImage, finished: true)
                        }
                        return
                    }
                    if let delegate = self.delegate {
                        delegate.xAnimationDidFinishLoop(self.animatedImage, loop: self.loopIndex)
                    }
                    self.currentFrameIndex = 0
                }
...
protocol XAnimatedImageViewDelegate: class {
